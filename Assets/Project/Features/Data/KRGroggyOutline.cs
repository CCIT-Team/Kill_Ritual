...
namespace KillRitual.Enemies
{
    ...
    public sealed class KRGroggyOutline : MonoBehaviour
    {
        ...
        [SerializeField] private Color _groggyColor = new Color(0.2f, 0.6f, 1f, 1f);  // 파랑

        [Tooltip("처형 가능 거리 안에 들어왔을 때 테두리 색상.")]
        [SerializeField] private Color _executableColor = new Color(1f, 0.5f, 0f, 1f); // 주황
        ...
        public void SetInRange(bool inRange)
        {
            ...
            if (_logStateChange)
                Debug.Log($"[KRGroggyOutline] {name}: SetInRange({inRange}) / IsGroggy={_isGroggy}");
