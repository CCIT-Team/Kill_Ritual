...
namespace VLB
{
    ...
    public class DynamicOcclusionDepthBuffer : DynamicOcclusionAbstractBase
    {
...
#if UNITY_EDITOR
            DestroyDepthCameraEditor();
            MarkMaterialAsDirty();
#else
            DestroyDepthCameraRuntime();
#endif
        }

        void DestroyDepthCameraRuntime()
        {
            if (m_DepthCamera)
            {
        ...
        void DestroyDepthCameraEditor()
        {
            ...
            var depthCamera = m_DepthCamera;
            var depthCameraGameObject = depthCamera.gameObject;
            var targetTexture = depthCamera.targetTexture;

            m_DepthCamera = null;

            // Prefab revert can crash if generated child objects are destroyed synchronously from OnDestroy, so delay the destroy.
            UnityEditor.EditorApplication.delayCall += () =>
            {
                if (depthCamera && depthCamera.targetTexture == targetTexture)
                ...
                if (depthCameraGameObject)
                    DestroyImmediate(depthCameraGameObject);
            };
        }
#endif
        ...
        void MarkMaterialAsDirty()
        {
            ...
            if (!Application.isPlaying && m_Master)
